...
contract Fogchain_2 is Mortal {
    ...
    constructor(uint seq_blk_interval, uint renew_price, uint unre_price) public {
    ...
    function get_current_seqnum() view public  
        returns (uint seqnum) 
    {
        uint cur_seqnum = (block.timestamp - TIME_START)/SEQUENCE_INV;
        return (cur_seqnum);
    }
    ...
    function settle() public returns (uint seqnum){
        ...
        if(power_station_addrs.length == 0){
            // event SettleEvent(uint indexed seqnum, uint indexed role, address indexed account_addr, uint consumption, uint output,  int expense);
            emit SettleEvent(cur_seqnum-1, POWER_STATION, address(0), 0, ps_sum, -int(ps_sum*UNRENEWABLE_PRICE));
        }
        else{
